    ...
    override func viewDidLoad() {
        ...
        reference.addSnapshotListener {[weak self] snapshot, _ in
            if let documents = snapshot?.documentChanges,
               let resultController = self?.fetchedResultController {
                self?.fireBaseConnectionHandler.receiveChannels(from: documents,
     ...
     func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
           let deleteAction = UIContextualAction(style: .normal, title: "Delete") { (_: UIContextualAction, _: UIView, success: (Bool) -> Void) in
            if let channel = self.getItem(by: indexPath) {
                self.fireBaseConnectionHandler.deleteChannel(with: channel, through: self.reference)
            }
               success(true)
           }
